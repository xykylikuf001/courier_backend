...
from ...utils.import_utils import import_string
...
class PluginsManager(PaymentInterface):
    ...
    def _load_plugin(
        ...
        db_config = None
        if plugin_class.PLUGIN_ID in db_configs_map:
            db_config = db_configs_map[plugin_class.PLUGIN_ID]
            plugin_config = db_config.configuration
            is_active = db_config.is_active
        else:
            plugin_config = plugin_class.DEFAULT_CONFIGURATION
            is_active = plugin_class.get_default_active()
        return plugin_class(
    ...
    async def _ensure_plugins_loaded(self, async_db: "AsyncSession"):
        global_db_config = await self._get_db_plugin_configs(async_db)
    ...
    @staticmethod
    async def _get_db_plugin_configs(async_db: "AsyncSession"):
        plugin_manager_configs = await plugin_config_repo.get_all(async_db)
    ...
    async def __run_method_on_plugins(
            self,
    ...
    @staticmethod
    async def __run_method_on_single_plugin(
        ...
        if inspect.iscoroutinefunction(plugin_method):
            returned_value = await plugin_method(*args, **kwargs, previous_value=previous_value)  # type:ignore
        else:
    ...
    async def initialize_payment(
            self, async_db: "AsyncSession", gateway, payment_data: dict, channel_slug: str
    ) -> Optional["InitializedPaymentResponse"]:
        method_name = "initialize_payment"
        default_value = None
        gtw = await self.get_plugin(gateway, async_db)
        if not gtw:
            return None

        return await self.__run_method_on_single_plugin(
            gtw,
            method_name,
    ...
    async def authorize_payment(
        ...
        return await self.__run_payment_method(
            gateway, "authorize_payment", payment_information, async_db=async_db
        )
    ...
    async def capture_payment(
        ...
        return await self.__run_payment_method(
            gateway, "capture_payment", payment_information, async_db=async_db
        )
    ...
    async def refund_payment(
        ...
        return await self.__run_payment_method(
            gateway, "refund_payment", payment_information, async_db=async_db
        )

    async def void_payment(
            self, async_db: "AsyncSession", gateway: str, payment_information: "PaymentData"
    ) -> "GatewayResponse":
        return await self.__run_payment_method(
            gateway, "void_payment", payment_information, async_db=async_db
        )
    ...
    async def confirm_payment(
        ...
        return await self.__run_payment_method(
            gateway, "confirm_payment", payment_information, async_db=async_db
        )
    ...
    async def process_payment(
        ...
        return await self.__run_payment_method(
            gateway, "process_payment", payment_information, async_db=async_db
        )
    ...
    async def list_payment_sources(
            self,
            gateway: str,
            customer_id: str,
            async_db: "AsyncSession"
        ...
        if gtw is not None:
            return await self.__run_method_on_single_plugin(
                gtw, "list_payment_sources", default_value, customer_id=customer_id, async_db=async_db
            )
    ...
    async def get_all_plugins(self, async_db: "AsyncSession", active_only=False):
        return await self.get_plugins(async_db, active_only=active_only)
    ...
    async def get_plugins(
        ...
        await self._ensure_plugins_loaded(async_db)
        plugins = self.all_plugins
        if active_only:
            plugins = [plugin for plugin in plugins if plugin.is_active]
    ...
    async def list_payment_gateways(
        ...
        plugins = await self.get_all_plugins(async_db, active_only=active_only)
    ...
    async def __run_payment_method(
            self,
            gateway: str,
            method_name: str,
        ...
        default_value = None
        plugin = await self.get_plugin(gateway, async_db)
        if plugin is not None:
            resp = await self.__run_method_on_single_plugin(
                plugin,
                method_name,
                previous_value=default_value,
    ...
    async def __run_plugin_method_until_first_success(
            self,
            method_name: str,
            *args,
            async_db: "AsyncSession",
        ...
        if plugins is None:
            plugins = await self.get_plugins(async_db, active_only=True)
        if plugins:
            for plugin in plugins:
                result = await self.__run_method_on_single_plugin(
                    plugin, method_name, None, *args, async_db=async_db
                )
    ...
    async def save_plugin_configuration(
        ...
        plugins = await self.get_plugins(async_db)

        for plugin in plugins:
            if plugin.PLUGIN_ID == plugin_id:
                plugin_configuration = await plugin_config_repo.first(
                    async_db=async_db,
                    params={"identifier": plugin_id}
                )
                if plugin_configuration is None:
                    plugin_configuration = await plugin_config_repo.create(
                        async_db=async_db, obj_in={
                            "identifier": plugin.PLUGIN_ID,
                            "name": plugin.PLUGIN_NAME,
                            "description": plugin.PLUGIN_DESCRIPTION,
                            "is_active": plugin.DEFAULT_ACTIVE,
                            "configuration": plugin.DEFAULT_CONFIGURATION
                        }
                    )
                configuration = await plugin.save_plugin_configuration(
                    plugin_configuration, cleaned_data, async_db
                )
                configuration.name = plugin.PLUGIN_NAME
                configuration.description = plugin.PLUGIN_DESCRIPTION
                plugin.is_active = configuration.is_active
                plugin.configuration = configuration.configuration
                return configuration

    async def get_plugin(
            self, plugin_id: str, async_db: "AsyncSession"
    ) -> Optional["BasePlugin"]:
        plugins = await self.get_plugins(async_db=async_db)
        for plugin in plugins:
