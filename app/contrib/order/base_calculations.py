...
from app.utils.prices import Money, zero_money, quantize_price
from app.contrib.discount import DiscountTypeChoices, DiscountValueTypeChoices
from app.contrib.discount.models import OrderDiscount
...
from .interface import OrderPricesData
...
def base_order_subtotal(order: "Order", lines: Iterable["OrderLine"]) -> Money:
    ...
    for line in lines:
        base_line_total = line.total_price_amount
...
def base_order_line_total(order_line: "OrderLine") -> OrderPricesData:
    ...
    order discounts.
    """
    price_with_line_discounts = order_line.total_price
    un_discounted_price = order_line.un_discounted_total_price
    return OrderPricesData(
        un_discounted_price=un_discounted_price,
...
def propagate_order_discount_on_order_prices(
        order: "Order",
        lines: Iterable["OrderLine"],
        order_discounts: Iterable["OrderDiscount"],
) -> tuple[Money, Money]:
    ...
    for order_discount in order_discounts:
        ...
        if order_discount.type == DiscountTypeChoices.voucher:
...
def assign_order_line_prices(line: "OrderLine", total_price: Money):
    line.total_price_amount = total_price
    ...
    if quantity > 0:
        ...
        un_discounted_unit_price = line.undiscounted_base_unit_price_amount
        line.un_discounted_unit_price_net_amount = undiscounted_unit_price
        line.un_discounted_unit_price_gross_amount = undiscounted_unit_price
...
def assign_order_prices(
    ...
    un_discounted_total = un_discounted_order_total(
        order, lines
    )
    order.un_discounted_total_amount = un_discounted_total.amount


def un_discounted_order_shipping(
        order: "Order",
        database_connection_name: str = settings.DATABASE_CONNECTION_DEFAULT_NAME,
) -> Money:
    """Return shipping price without any discounts."""
    # TODO: add un_discounted_shipping_price field to order model.
...
def un_discounted_order_subtotal(order: "Order", lines: Iterable["OrderLine"]) -> Money:
    """Return order subtotal without any discounts."""
    currency = order.currency
    subtotal = zero_money(currency)
    for line in lines:
        un_discounted_line_total = line.undiscounted_unit_price.net * line.quantity
        subtotal += un_discounted_line_total
    return quantize_price(subtotal, currency)


def un_discounted_order_total(
        order: "Order",
        lines: Iterable["OrderLine"],
        database_connection_name: str = settings.DATABASE_CONNECTION_DEFAULT_NAME,
) -> Money:
    """Return order total without any discounts."""
    subtotal = un_discounted_order_subtotal(order, lines)
