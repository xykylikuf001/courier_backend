...
def _pytz_imported():
    """
    Detects whether pytz has been imported without importing pytz.
...
def today(is_midnight: Optional[bool] = True) -> datetime:
    value = now()
    if is_midnight:
...
def tomorrow(is_midnight: Optional[bool] = True) -> datetime:
    value = today(is_midnight)
