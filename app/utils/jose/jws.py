...
def sign(
    ...
    Args:
        ...
        key (str | bytes | dict): The key to use for signing the claim set. Can be
    ...
    Examples:
        ...
        >>> sign({'a': 'b'}, 'secret', algorithm='HS256')
...
def verify(
    ...
    Examples:
        ...
        >>> token_str = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhIjoiYiJ9.jiMyrsmD8AoHWeQgmxZ5yq8z0lXS67_QGs52AzC8Ru8'
        >>> verify(token_str, 'secret', algorithms='HS256')
...
def _encode_header(algorithm: str, additional_headers: Optional[dict] = None) -> bytes:
...
def _encode_payload(payload: Union[str, dict]) -> bytes:
...
def _sign_header_and_claims(encoded_header: bytes, encoded_claims, algorithm: str, key: Union[str, dict]) -> str:
...
def _load(jwt: str) -> Tuple[Mapping, bytes, List[bytes], bytes]:
