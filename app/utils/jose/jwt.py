...
def encode(
        claims: dict,
        key: Union[str, dict],
        algorithm: Optional[str] = ALGORITHMS.HS256,
    ...
    Examples:
        ...
        >>> encode({'a': 'b'}, 'secret', algorithm='HS256')
...
def decode(
    ...
    Examples:
        ...
        >>> payload_str = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhIjoiYiJ9.jiMyrsmD8AoHWeQgmxZ5yq8z0lXS67_QGs52AzC8Ru8'
        >>> decode(payload_str, 'secret', algorithms='HS256')
    ...
    try:
        payload = jws.verify(token, key, algorithms, verify_it=verify_signature)
...
def get_unverified_claims(token: str) -> Mapping:
...
def _validate_iat(claims: Mapping):
...
def _validate_nbf(claims: Mapping, leeway: Optional[int] = 0):
...
def _validate_exp(claims: Mapping, leeway: Optional[int] = 0):
...
def _validate_aud(claims: Mapping, audience: Optional[str] = None):
...
def _validate_iss(claims: Mapping, issuer: Optional[str] = None):
...
def _validate_sub(claims: Mapping, subject: Optional[str] = None):
...
def _validate_jti(claims: Mapping):
...
def _validate_at_hash(claims: Mapping, access_token: str, algorithm: str):
    """
