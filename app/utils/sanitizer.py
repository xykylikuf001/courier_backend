...
class Sanitizer:
    def __init__(
            ...
            whitespace=None,
            keep_typographic_whitespace: Optional[bool] = False,
            add_nofollow: Optional[bool] = False,
            autolink: Optional[bool] = False,
    ):

        # convert span elements into em/strong if a matching style rule
        # has been found. strong has precedence, strong & em at the same
        # time is not supported
        self.element_preprocessors = [
            bold_span_to_strong,
            italic_span_to_em,
            tag_replacer("b", "strong"),
            tag_replacer("i", "em"),
        ...
        self.element_postprocessors = []
        self.sanitize_href = sanitize_href

        # Allow iterables of any kind, not just sets.
        self.tags = coerce_to_set(tags) if tags else {
            "a", "h1", "h2", "h3", "h4", "h5", "h6",
            "strong", "em", "p", "ul", "ol", "li",
            "br", "sub", "sup", "hr", "div", "img", "span",
            "figure", "figcaption"
        }

        self.empty = coerce_to_set(empty) if empty else {"hr", "a", "br"}
        self.separate = coerce_to_set(separate) if separate else {"a", "p", "li"}
        self.whitespace = coerce_to_set(whitespace) if whitespace else {"br"}
        if attributes:
            self.attributes = {
        ...
        self.keep_typographic_whitespace = keep_typographic_whitespace
    ...
    def sanitize(self, html):
        ...
        if self.autolink is True:
            lxml_html_clean.autolink(doc)
        elif isinstance(self.autolink, dict):
            lxml_html_clean.autolink(doc, **self.autolink)
