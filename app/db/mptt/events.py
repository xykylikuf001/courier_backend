...
def _insert_subtree(
    ...
    connection.execute(
        table.update().where(table_pk.in_(subtree)).values(
    ...
    connection.execute(
        table.update().where(
            ...
            lft=case((table.c.lft > left_sibling['lft'], table.c.lft + node_size), else_=table.c.lft)
        )
    )
...
def mptt_before_insert(mapper, connection, instance):
    ...
    if instance.parent_id is None:
        ...
        connection.execute(
            table.update()
            .where(and_(table.c.rgt >= parent_pos_right, table.c.tree_id == parent_tree_id))
            .values(
                lft=case((table.c.lft > parent_pos_right, table.c.lft + 2), else_=table.c.lft),
                rgt=case((table.c.rgt >= parent_pos_right, table.c.rgt + 2), else_=table.c.rgt)
            )
        )
...
def mptt_before_update(mapper, connection, instance):
    ...
    if instance.parent_id is not None:
        ...
        if left_sibling_tree_id or left_sibling_tree_id == 0:
            ...
            connection.execute(
                table.update().where(table.c.tree_id > left_sibling_tree_id).values(
